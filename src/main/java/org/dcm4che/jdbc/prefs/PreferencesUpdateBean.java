 ...
 */
public class PreferencesUpdateBean {

    private static final Logger LOG = Logger.getLogger(PreferencesUpdateBean.class);

    private EntityManager em;
    private static final String datasource = System.getProperty("jdbc.prefs.datasource");

    private static PreferencesUpdateBean singleton;

    public static PreferencesUpdateBean getInstance() {
        if (singleton == null)
            singleton = new PreferencesUpdateBean();
        return singleton;
    }

    public PreferencesUpdateBean() {
        if (datasource == null)
